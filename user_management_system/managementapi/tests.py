...
class CustomUserAPITestCase(APITestCase):
    ...
    def test_update_method_fail(self):
        ...
        self.client.login(username='ranu', password='12334')
        response = self.client.put(reverse('user_update', kwargs={'pk': 23}), format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_retrieve_method_success(self):
        """This is method to test working of retrieve API for success"""
        self.client.login(username='ranu', password='12334')
        response = self.client.get(reverse('user_retrieve', kwargs={'pk': self.user.id}), format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_retrieve_method_fail(self):
        """"This is method to test working of retrieve API for fail"""
        self.client.login(username='ranu', password='12334')
        response = self.client.get(reverse('user_retrieve', kwargs={'pk': 34}), format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_update_method_success(self):
        ...
        self.client.login(username='ranu', password='12334')
        response = self.client.put(reverse('user_update', kwargs={'pk': self.user.id}), data, format='json')
